...
def ensure_date_str(date_value) -> str:
    ...
    If the input is an integer, convert it to a string with leading zeros.
    """
...
def is_valid_4d(four_d: str) -> bool:
    """Validate the format of 4D_Number."""
    return re.match(r'^4D\d+$', four_d.upper()) is not None
...
def get_nominal_records():
    ...
    """
    return SHEET_NOMINAL.get_all_records()
...
def get_parade_records():
    ...
    """
    return SHEET_PARADE.get_all_records()
...
def get_conduct_records():
    ...
    """
    return SHEET_CONDUCTS.get_all_records()
    ...
    for row in parade_data:
        ...
        if normalize_name(c) == normalize_name(company):
            ...
            if start_d <= date_obj <= end_d:
                ...
                if not is_valid_4d(four_d):
                    logger.warning(f"Invalid 4D_Number format: {four_d}")
                    continue
...
def has_overlapping_status(four_d: str, new_start: datetime, new_end: datetime) -> bool:
    ...
    parade_data = get_parade_records()
    for row in parade_data:
        if row.get("4D_Number", "").strip().upper() == four_d.upper():
            start_date = row.get("Start_Date_DDMMYYYY", "")
            end_date = row.get("End_Date_DDMMYYYY", "")
            
            # Ensure dates are strings with leading zeros
            start_date = ensure_date_str(start_date)
            end_date = ensure_date_str(end_date)
...
if feature == "Add Conduct":
    ...
    if st.button("Finalize Conduct"):
        ...
        existing_4ds = {row.get("4D_Number", "") for row in existing_nominal}

        new_people = []
        all_outliers = []

        for row in edited_data:
            four_d = row.get("4D_Number", "").strip()
            name_ = row.get("Name", "").strip()
            is_outlier = row.get("Is_Outlier", False)
            status_desc = row.get("StatusDesc", "").strip()

            # Validate 4D_Number format
            if not is_valid_4d(four_d):
                st.error(f"Invalid 4D_Number format: {four_d}. Skipping.")
        ...
        for (nm, fd, c_) in new_people:
            SHEET_NOMINAL.append_row([nm, fd, c_, 14, ""])  # Initialize leaves
            logger.info(f"Added new person to Nominal_Roll: {nm}, {fd}, {c_}.")
        ...
        logger.info(f"Appended Conduct: {date_str}, {comp}, {cname}, Total: {total_strength}, Participating: {participating}, Outliers: {outliers_str}, Submitted_By: {submitted_by}")
...
# ------------------------------------------------------------------------------
    ...
    if st.session_state.parade_table:
        ...
        if st.button("Update Parade State"):
            ...
            for row in edited_data:
                four_d = row['4D_Number'].strip()
                status_val = row.get('Status', '').strip()
                start_val = row.get('Start_Date', '').strip()
                end_val = row.get('End_Date', '').strip()
                ...
                try:
                    ...
                    logger.error(f"Invalid date format for {four_d}: Start={start_val}, End={end_val}.")
                    continue

                if status_val.lower() == "leave":
                    # Calculate number of leaves used
                    if start_val != end_val:
                        dates_str = f"{start_val}-{end_val}"
                    else:
                        dates_str = start_val
                ...
                SHEET_PARADE.append_row([c, four_d, status_val, start_val, end_val, submitted_by])
                rows_updated += 1
                logger.info(f"Appended Parade_State for {four_d}: Status={status_val}, Start={start_val}, End={end_val}, Submitted_By={submitted_by}")
...
# ------------------------------------------------------------------------------
    ...
    with query_tabs[0]:
        ...
        if st.button("Get Statuses", key="btn_query_person"):
            four_d_input_clean = four_d_input.strip().upper()
            if not four_d_input_clean:
                st.error("Please enter a valid 4D Number.")
                st.stop()

            if not is_valid_4d(four_d_input_clean):
                st.error(f"Invalid 4D_Number format: {four_d_input_clean}. It should start with '4D' followed by digits.")
                logger.error(f"Invalid 4D_Number format: {four_d_input_clean}.")
                st.stop()
    ...
    with query_tabs[1]:
        ...
        if st.button("Get Outliers", key="btn_query_outliers"):
            company_query = comp_q.strip()
            conduct_query = cond_q.strip()
